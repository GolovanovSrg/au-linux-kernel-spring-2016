    ...
    const char __user *write_user_buf, size_t write_size, loff_t *fpos)
{
     ...
     * 1. Приходит прерывание, драйвер продолжает свою работу, очищяя kdma_buf
     * 2. Переключаемся на девайс, который начинает выполнение записи и записывает
     * данные в очищенный буфер.
     */
